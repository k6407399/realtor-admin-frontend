...
function ManageProperty() {
  ...
  const renderDetails = (property) => {
    // Flatten and select specific fields for display
    const selectedFields = ["location", "area", "pricePerSqft", "totalPrice", "city"];
          ...
          <TableBody>
            {properties.map((property) => (
              <TableRow key={property.id}>
                <TableCell>{property.id}</TableCell>
                <TableCell>{propertyType}</TableCell>
                <TableCell>{JSON.stringify(property.details)}</TableCell>
                <TableCell>
                  <Select
                    value={statuses[property.id] || property.status}
                    onChange={(e) => handleStatusChange(property.id, e.target.value)}
